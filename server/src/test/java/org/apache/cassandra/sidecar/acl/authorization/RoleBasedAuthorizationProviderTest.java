...
{
    CassandraRoleAuthorizationsCache mockRolePermissionsCache;
    SidecarRoleAuthorizationsProvider mockSidecarPermissionsProvider;

    @BeforeEach
    void setup()
    {
        mockRolePermissionsCache = mock(CassandraRoleAuthorizationsCache.class);
        mockSidecarPermissionsProvider = mock(SidecarRoleAuthorizationsProvider.class);
    }
    ...
    void testAuthorizationsFetched(VertxTestContext testContext)
    {
        IdentityToRoleCache mockIdentityToRoleCache = mock(IdentityToRoleCache.class);
        when(mockIdentityToRoleCache.get("spiffe://cassandra/sidecar/test_user")).thenReturn("test_role");
        CassandraRoleAuthorizationsCache mockRolePermissionsCache = mock(CassandraRoleAuthorizationsCache.class);
        when(mockRolePermissionsCache.getAuthorizations("test_role"))
        .thenReturn(Collections.singleton(CassandraActions.CREATE.toAuthorization()));
        SidecarRoleAuthorizationsProvider mockSidecarPermissionsProvider = mock(SidecarRoleAuthorizationsProvider.class);
        when(mockSidecarPermissionsProvider.getAuthorizations("test_role"))
        .thenReturn(Collections.singleton(SidecarActions.CREATE_SNAPSHOT.toAuthorization()));
        RoleBasedAuthorizationProvider authorizationProvider = new RoleBasedAuthorizationProvider(mockIdentityToRoleCache,
                                                                                                  mockRolePermissionsCache,
                                                                                                  mockSidecarPermissionsProvider);
        User user = MutualTlsUser.fromIdentities(Collections.singletonList("spiffe://cassandra/sidecar/test_user"));
